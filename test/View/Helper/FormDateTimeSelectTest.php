...
final class FormDateTimeSelectTest extends AbstractCommonTestCase
{
    ...
    public function testRendersDatesWithEsCLLocaleDatePattern(): void
    {
        ...
        // The time portion of the pattern depends on the ICU version in use (`HH:mm` in older
        // releases, `h:mm a` in newer ones), so only the date portion is asserted here.
        $hourPosition = strpos($markup, '<select name="hour"');
        self::assertNotFalse($hourPosition);
        $datePortion = substr($markup, 0, $hourPosition);
